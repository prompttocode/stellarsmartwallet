...
import { Modal, View, Text, TouchableOpacity, StyleSheet, Platform, TextInput, Animated, PanResponder } from 'react-native';
...
export function WalletManagerModal({
  ...
  useEffect(() => {
    const list = walletState.wallets?.length > 0 ? walletState.wallets : (walletState.wallet ? [walletState.wallet] : []);
    setData(list);
  ...
  const renderItem = ({ item, drag, isActive, getIndex }: RenderItemParams<Wallet>) => {
    const isSelected = item.id === activeWalletId;
    const displayName = item.displayName || `Stellar Wallet ${getIndex() !== undefined ? getIndex()! + 1 : ''}`;
    ...
    return (
      <ScaleDecorator>
        <TouchableOpacity
          ...
          style={[styles.item, isSelected && !isEditing ? styles.itemSelected : null]}
        >
          ...
          <View style={styles.itemBody}>
            <View style={styles.itemTitleRow}>
              ...
              {isSelected && !isEditing && <View style={styles.tag}><Text style={styles.tagText}>Active</Text></View>}
            </View>
          ...
          {isEditing ? (
            <View style={styles.editActions}>
              <TouchableOpacity onPress={() => handleRename(item)} style={styles.actionBtn}>
          ...
          <View style={styles.header}>
            <TouchableOpacity onPress={() => setIsEditing(!isEditing)} style={styles.headerBtn}>
              <View style={styles.doneBtn}>
                <Text style={styles.headerBtnText}>{isEditing ? 'Done' : 'Edit'}</Text>
              </View>
            </TouchableOpacity>
            ...
            <View style={styles.promptOverlay}>
              <View style={styles.promptBox}>
                ...
                <View style={styles.promptActions}>
                  <TouchableOpacity style={styles.promptBtn} onPress={() => setRenamingWallet(null)}>
                    <Text style={styles.promptBtnText}>Hủy</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.promptBtnPrimary} onPress={submitRename}>
...
const styles = StyleSheet.create({
  ...
  sheetContainer: { backgroundColor: '#131A22', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingBottom: Platform.OS === 'ios' ? 40 : 20, maxHeight: Platform.OS === 'ios' ? 700 : 650 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingBottom: 24, paddingTop: 10 },
  headerBtn: { minWidth: 60, alignItems: 'flex-start' },
  headerBtnRight: { minWidth: 60, alignItems: 'flex-end' },
  doneBtn: { backgroundColor: '#1E293B', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 },
  closeBtn: { backgroundColor: '#1E293B', width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  headerBtnText: { color: '#FFF', fontSize: 14, fontWeight: '600' },
  title: { color: '#FFF', fontSize: 20, fontWeight: '700' },
  listContainer: { maxHeight: 400 },
  list: { paddingHorizontal: 20 },
  item: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1E293B', padding: 16, borderRadius: 16, marginBottom: 12 },
  itemSelected: { borderColor: '#0F8EA3', borderWidth: 1 },
  iconBox: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#334155', alignItems: 'center', justifyContent: 'center' },
  ...
  tag: { backgroundColor: '#0F8EA3', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  ...
  addButton: { backgroundColor: '#1E293B', padding: 16, borderRadius: 24, alignItems: 'center' },
  addButtonText: { color: '#FFF', fontSize: 16, fontWeight: '600' },
  promptOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' },
  promptBox: { width: '80%', backgroundColor: '#1E293B', borderRadius: 16, padding: 20 },
  promptTitle: { color: '#FFF', fontSize: 18, fontWeight: '700', marginBottom: 8 },
  promptDesc: { color: '#8A9AA3', fontSize: 14, marginBottom: 16 },
  promptInput: { backgroundColor: '#0F172A', color: '#FFF', borderRadius: 8, padding: 12, fontSize: 16, marginBottom: 20, borderWidth: 1, borderColor: '#334155' },
  ...
  promptBtnPrimary: { backgroundColor: '#0F8EA3', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 8 },
