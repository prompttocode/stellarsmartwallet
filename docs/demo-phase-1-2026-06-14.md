...
- [Trạng thái Worker](https://api.getstellar.shop/api/health)
- [Các mạng được hỗ trợ](https://api.getstellar.shop/api/networks)
- [Các tài sản trên Testnet](https://api.getstellar.shop/api/assets?network=testnet)
...
Backend đã gỡ cơ chế dùng email dự phòng và các route tạo session/account demo. `/api/session`, `/api/session/status` và tiện ích tạo receiver Testnet đều yêu cầu xác thực bằng Privy identity token/Bearer token.
