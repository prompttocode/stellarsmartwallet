...
| `POST /api/wallets` | Tạo ví Stellar mới cho account hiện tại. Mobile tạo Privy embedded Stellar wallet rồi gửi `wallet.id/address`; worker xác minh địa chỉ ví thuộc đúng Privy user trước khi lưu. Testnet có thể auto fund, mainnet phải nạp XLM thật. | `201 SessionResponse` | `400` ví client không khớp Privy, `401` thiếu/hết hạn token, `403` ví không thuộc user, `404` account không tồn tại, `409` Privy chưa link ví kịp, `500/502` lỗi Privy/backend. |
