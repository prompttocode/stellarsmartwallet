...
| `POST /api/wallets` | Tạo ví Stellar mới cho account hiện tại. Mobile tạo Privy embedded Stellar wallet rồi gửi `wallet.id/address`; worker lấy lại wallet từ Privy và xác minh đúng address + chain trước khi lưu. Testnet có thể auto fund, mainnet phải nạp XLM thật. | `201 SessionResponse` | `400` ví client không khớp Privy hoặc không phải Stellar, `401` thiếu/hết hạn token, `404` account không tồn tại, `500/502` lỗi Privy/backend. |
